...
to_from_SIN = pd.read_excel(
    r'C:\Users\amrsa\OneDrive - University of Surrey\PhD\ATM\Frankie\Publication\Code\Data\SIN-SYD\to_from_SIN_6.xlsx', sheet_name=1)
...
#             return a[a['Leg Operating Airline'] == 'D7']['ARPP($)'].to_numpy()
#         elif row['Leg Operating Airline'] == 'TR':
#             return a[a['Leg Operating Airline'] == 'TR']['ARPP($)'].to_numpy()
#         elif row['Cabin Class'] == 'Discount Coach':
#             airline = row['Leg Operating Airline']
#             if airline != 'JQ' :
#                 return a[a['Leg Operating Airline'] == airline]['ARPP($)'].to_numpy()
#             else:
#                 return 0
#         else:
#             return 0        
...
def gross_yield(df):
    gy = df.groupby(['Leg Origin Airport', 'Leg Destination Airport',
                     'Leg Operating Airline', 'Month']).sum().reset_index() # this didn't have cabin class 
    gy = gy[['Leg Origin Airport', 'Leg Destination Airport',
               'Leg Operating Airline', 'Month', 'Passengers', 'R_dir', 'R_con']] # this didn't have cabin class
    gy['Yield'] = (gy['R_dir'] + gy['R_con']) / gy['Passengers']
...
def airline_type(row):
    if row.loc['Leg Operating Airline'] == 'TR' or row.loc['Leg Operating Airline'] == 'D7' :
        return 'LHLCC'
    else:
        return 'FSNC'
...
def calculate(df, l_df, name):
    ...
    gy['R_total'] = gy['Net_Yield']*gy['Load Factor'] * \
    ...
    RE.drop(['B_wa'], axis=1, inplace=True)

    print("Retrieving average Block hours from datafram B...")
    ...
    B = B[['Leg Operating Airline', 'B_wa', 'Yield']]
...
# Yield plot
results_REB['Y/P/B'] = results_REB['Yield_y'] / results_REB['B_wa']
...
reb = ax.bar(x, y, w, color=[l,f,f,f,f,l,f,f,l,l,f,l,f,l,f,f,f,f,l,l,f,f,f,f,f,f,l,f,l,f,l,f,l,f])

ax.set_ylabel('REB - Revenue/ e-seat/ block hour (US$)')
ax.set_xticks(x)
ax.set_xticklabels(airline_names)
ax.set_ylim([0,ylim])
ax.set_xlim([0-w, 33+w])
...
ax.plot([22.5,22.5],[0,ylim], color='k',linewidth=lw)
ax.plot([26.5,26.5],[0,ylim], color='k',linewidth=lw)
ax.plot([28.5,28.5],[0,ylim], color='k',linewidth=lw)
ax.plot([31.5,31.5],[0,ylim], color='k',linewidth=lw)
...
ax.annotate('SIN-MEL',xy=(20.5,cpp),horizontalalignment='center', weight='bold', fontsize=fscp)
ax.annotate('SIN-SYD',xy=(24.5,cpp),horizontalalignment='center', weight='bold', fontsize=fscp)
ax.annotate('KUL-SYD',xy=(27.5,cpp),horizontalalignment='center', weight='bold', fontsize=fscp)
ax.annotate('KUL-JED',xy=(30,cpp),horizontalalignment='center', weight='bold', fontsize=fscp)
ax.annotate('SIN-JED',xy=(32.5,cpp),horizontalalignment='center', weight='bold', fontsize=fscp)
...
ax.annotate('6080 km',xy=(20.5,dss),horizontalalignment='center', fontsize=fs, style='italic')
ax.annotate('6340 km',xy=(24.5,dss),horizontalalignment='center', fontsize=fs, style='italic')
ax.annotate('6362 km',xy=(27.5,dss),horizontalalignment='center', fontsize=fs, style='italic')
ax.annotate('7087 km',xy=(30,dss),horizontalalignment='center', fontsize=fs, style='italic')
ax.annotate('7379 km',xy=(32.5,dss),horizontalalignment='center', fontsize=fs, style='italic')
...
ax.plot([18.5,22.5],[avgs[6],avgs[6]], color='k',linewidth=lwavgs, linestyle='dashed')
ax.plot([22.5,26.5],[avgs[7],avgs[7]], color='k',linewidth=lwavgs, linestyle='dashed')
ax.plot([26.5,28.5],[avgs[8],avgs[8]], color='k',linewidth=lwavgs, linestyle='dashed')
ax.plot([28.5,31.5],[avgs[9],avgs[9]], color='k',linewidth=lwavgs, linestyle='dashed')
ax.plot([31.5,33+w],[avgs[10],avgs[10]], color='k',linewidth=lwavgs, linestyle='dashed')
...
airline_type = list(yield_type['Airline Type'])
...
axy.set_xticks(x)
axy.set_xticklabels(airline_type)
axy.set_ylim([0,80])
...
axp.set_xticks(x)
axp.set_xticklabels(airline_type)
axp.set_ylim([0,30])
...
#calculated by hand
labels = 'Passengers REB Passengers REB'.split()
...
axc.annotate('Connecting Passengers',xy=(0,55),horizontalalignment='center', fontsize=10, rotation=90)
axc.annotate('Connecting Revenue',xy=(1,64),horizontalalignment='center', fontsize=10, rotation=90)
axc.annotate('C.P',xy=(2,97),horizontalalignment='center', fontsize=10)
axc.annotate('C.R',xy=(3,97),horizontalalignment='center', fontsize=10)
...
# gy = gy.drop(gy[gy['Leg Operating Airline'] not in 'SQ QF TR EK BA TR SV MH OD D7 CA NH JL'.split()].index)
...
axl.set_xticks(x)
axl.set_xticklabels(airline_type)
axl.set_ylim([0,100])
...
axl.annotate('- 6.3%p',xy=(0.5,96),horizontalalignment='center')
...
lf_plot = axs.bar(x,y,w2)

axs.set_xticks(x)
axs.set_xticklabels(airline_type)
axs.set_ylim([0,100])
axs.set_xlim([0-w2,1+w2])
axs.bar_label(lf_plot, padding=3, fmt='%.1f')

#Arrow
axs.plot([0,0,1],[72,98,98], color='k')
axs.arrow(1,98,0,-4,width=0.01,head_length=2,head_width=0.2, color='k')
axs.annotate('+25.6%p',xy=(0.5,94),horizontalalignment='center')
fig.tight_layout()
plt.show()
...
ax.set_ylabel('REB - Revenue/ e-seat/ block hour (US$)')
ax.set_xticks(x)
ax.set_xticklabels(airline_type)
ax.set_ylim([0,30])
...
ax.annotate('- 28%',xy=(0.5,27.5),horizontalalignment='center')


# # wishes for the future:
# * copy this entire file and adjust it to run
# * clean it up and keep what is necessary 
# * re-organise folder and subsequently paths 
# * create an excel sheet than takes in the airline names, and o-d's 
# * if od file not present, prompt the user to download them from either OAG or SABRE
# * locate where in the code a step is not automated or requires user-intervention
# * automate them 
# * print the graphs into a file in both .eps and .png 
# * make it distributable for Frankie and students to use. 
# * make a readme.txt or manual.txt file. Maybe Word would be easier for the non-coder 
